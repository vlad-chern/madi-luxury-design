...
const ProductImageGallery = ({ mainImage, images = [] }: ProductImageGalleryProps) => {
  ...
  // Объединяем главное изображение с остальными
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative group">
        <div 
          className="aspect-square bg-cover bg-center rounded-lg relative overflow-hidden"
          style={{
      ...
      {/* Галерея thumbnail изображений */}
      {allImages.length > 1 && (
        <div className="space-y-4">
          <h4 className="text-sm font-medium text-gray-400">Galería</h4>
          <Carousel className="w-full">
            <CarouselContent className="-ml-2">
              {allImages.map((image, index) => (
                <CarouselItem key={index} className="pl-2 basis-1/4">
                  <div
                    className={`aspect-square bg-cover bg-center rounded-lg cursor-pointer transition-all duration-200 border-2 ${
