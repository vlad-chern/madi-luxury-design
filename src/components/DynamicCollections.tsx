...
const DynamicCollections = () => {
  ...
  const getLayoutForCategory = (index: number, total: number) => {
    if (total === 1) return 'lg:col-span-3';
    if (total === 2) return index === 0 ? 'lg:col-span-2' : 'lg:col-span-1';
    if (total >= 3) {
  ...
  const getAspectForCategory = (index: number, total: number) => {
    if (total === 1) return 'aspect-[21/9]';
    if (total === 2) return index === 0 ? 'aspect-[16/10]' : 'aspect-square lg:aspect-auto lg:h-full';
    if (total >= 3) {
      if (index === 0) return 'aspect-[16/10]';
      return 'aspect-square lg:aspect-auto lg:h-full';
    }
    return 'aspect-video';
  };
  ...
  // Renderizar primera fila (hasta 3 categorías)
  ...
  return (
    <div className="container mx-auto px-6">
      ...
      <div className="space-y-8">
        {/* Primera fila con layout especial */}
        {firstRowCategories.length > 0 && (
          <div className="grid lg:grid-cols-3 gap-8">
            {firstRowCategories.map((category, index) => (
              <div key={category.id} className={`group ${getLayoutForCategory(index, firstRowCategories.length)}`}>
                <div className="relative overflow-hidden rounded-xl">
                  <div 
                    className={`${getAspectForCategory(index, firstRowCategories.length)} bg-cover bg-center cursor-pointer transition-transform duration-500 group-hover:scale-105`}
                    style={{
                  ...
                  >
                    ...
                    <div className={`absolute bottom-0 left-0 right-0 ${index === 0 && firstRowCategories.length >= 2 ? 'p-8' : 'p-6'}`}>
                      <div className="flex items-center gap-3 mb-4">
                        <div className={`${index === 0 && firstRowCategories.length >= 2 ? 'w-12 h-12' : 'w-10 h-10'} bg-[rgb(180,165,142)] rounded-full flex items-center justify-center`}>
                          <span className={`text-[rgb(14,14,14)] font-bold ${index === 0 && firstRowCategories.length >= 2 ? 'text-lg' : ''}`}>
                            {String(index + 1).padStart(2, '0')}
                          </span>
                        </div>
                        <h3 className={`${index === 0 && firstRowCategories.length >= 2 ? 'text-3xl' : 'text-2xl'} font-bold text-white`}>
                          {category.name}
                        </h3>
                      </div>
                      <p className={`text-gray-200 mb-6 ${index === 0 && firstRowCategories.length >= 2 ? 'text-lg max-w-lg' : ''}`}>
                        {category.description || 'Diseño exclusivo y funcional para su hogar'}
                      </p>
                      <Button 
                        className={`${index === 0 && firstRowCategories.length >= 2 
                          ? 'bg-[rgb(180,165,142)] text-[rgb(14,14,14)] hover:bg-[rgb(160,145,122)]' 
                          : index === 2 && firstRowCategories.length >= 3
                        ...
                        onClick={() => handleCategoryClick(category.slug)}
                      >
                        {index === 0 && firstRowCategories.length >= 2 ? `Explorar ${category.name} →` : 'Ver Más'}
                      </Button>
                    </div>
                  </div>
                </div>
        ...
        {/* Categorías adicionales en formato wide */}
